...
import { MapPin, Navigation, ExternalLink, Clock, Users, Layers, Info } from 'lucide-react';
...
export const MapView: FC<MapViewProps> = ({ searchQuery }) => {
  ...
  const [showRoomDirectory, setShowRoomDirectory] = useState<boolean>(false);
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="bg-zinc-900/70 border border-zinc-800 rounded-lg p-3.5 text-xs text-zinc-300 leading-relaxed flex items-start gap-2.5">
        ...
        <div>
          <span className="font-semibold text-zinc-100 block mb-0.5">
            Einsatzplan: Pünktlichkeit & Treffzeiten
          </span>
          <span>
      ...
      {/* Toggle: Gebäude 29 Raum-Verzeichnis */}
      <div className="pt-4 border-t border-zinc-800">
        <button
          onClick={() => setShowRoomDirectory(!showRoomDirectory)}
          className="flex items-center justify-between w-full p-3 rounded-lg bg-zinc-900/60 border border-zinc-800 hover:border-zinc-700 text-xs font-medium text-zinc-300 cursor-pointer transition-colors"
        >
          <span className="flex items-center gap-2">
            <Layers className="w-4 h-4 text-zinc-400" />
            <span>Gebäude 29 Raum-Verzeichnis nach Etagen {showRoomDirectory ? 'ausblenden' : 'einblenden'}</span>
          </span>
          <span className="text-zinc-500 font-mono text-[11px]">
            {showRoomDirectory ? '▲ Schließen' : '▼ Öffnen'}
          </span>
        </button>

        {showRoomDirectory && (
          <div className="mt-3 overflow-x-auto rounded-lg border border-zinc-800 bg-zinc-900/50">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="border-b border-zinc-800 bg-zinc-900/90 text-zinc-400 font-medium">
